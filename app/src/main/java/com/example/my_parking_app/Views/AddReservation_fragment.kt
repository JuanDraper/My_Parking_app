...
class AddReservationFragment : Fragment(R.layout.fragment_add_reservation) {
    ...
    private val viewModel by lazy { activity?.let { AddViewModelProvider(it).get(AddViewModel::class.java) } }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.backButton.setOnClickListener {
            binding.root.findNavController().navigate(R.id.action_reservationsFragment_to_parkingLotsFragment) // switching screen to
        ...
        binding.startDateButton.setOnClickListener { pickStartDateTime() }

        binding.endDateButton.setOnClickListener { pickEndDateTime() }
        binding.authCodeButton.setOnClickListener { authCode = binding.authCodeButton.text.toString() }

        binding.saveButton.setOnClickListener { viewModel!!.addReservation(ReservationDetails("", startDate.timeInMillis, endDate.timeInMillis, authCode, parkinLot)) }
        viewModel?.addReservationLiveData?.observe(viewLifecycleOwner) { added ->
            when (added) {
                States.StatusSuccessful -> {
    ...
    private fun pickStartDateTime() {
        ...
        TimePickerDialog(activity, timeListener, startDate.get(Calendar.HOUR_OF_DAY), startDate.get(Calendar.MINUTE), false).show()

        DatePickerDialog(requireContext(), dateListener, startDate.get(Calendar.YEAR), startDate.get(Calendar.MONTH), startDate.get(Calendar.DAY_OF_MONTH)).show()
    }
    ...
    private fun pickEndDateTime() {
        ...
        TimePickerDialog(activity, timeListener, endDate.get(Calendar.HOUR_OF_DAY), endDate.get(Calendar.MINUTE), false).show()

        DatePickerDialog(requireContext(), dateListener, endDate.get(Calendar.YEAR), endDate.get(Calendar.MONTH), endDate.get(Calendar.DAY_OF_MONTH)).show()
    }
}